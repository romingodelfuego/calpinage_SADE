...
class PIECE :   
    ...
    def __dataPropagation(self,selfConn,pieceRef,pieceRefConn):
        if pieceRef.__dict__["conn"+str(pieceRefConn)+"Cat"] !="BRIDE": #si pieceRef n'est pas une bride
            self.typePiece = pieceRef.typePiece if pieceRef.typePiece else input("CAUSE LIAISON NORMAL :Quel type de piece:\tSTD\tEXPRESS\t")
            self.verPiece = pieceRef.verPiece if pieceRef.typePiece else input("CAUSE LIAISON NORMAL :Quel verrouillage de piece:\tVI\tVE\tNone\t")    
            ...
            for attr_name in connexCat: ## On vérifie si il existe des connexions sur la piece autre que bride
                if self.__dict__[attr_name]!="BRIDE": #Si il en existe il faut demander quelle type,ver
                    self.typePiece = input("CAUSE BRIDE :Quel type de piece:\tSTD\tEXPRESS\t")
                    self.verPiece = input("CAUSE BRIDE :Quel verrouillage de piece:\tVI\tVE\tNone\t")
...
class Constructor :   
    def __init__(self):
        self.listAuthorizedCatRef={"BRIDE":["BRIDE"],
                            "EMBOITEMENT":["BOUT MALE"],
                            "BOUT MALE":["EMBOITEMENT"]}

        
    def builtAll(self,*listMotMontage):
        listMontage=[[self.createPiece(nom,db_Connexions) for nom in montageMot] for montageMot in listMotMontage]
        self.associator(self.listAuthorizedCatRef,listMontage)

    def createPiece(self,nom,dataBasePandas):
        nom,connexionType,propaConn= self.searchPiece(nom,dataBasePandas)
        return PIECE(nom,connexionType,propaConn)
    
    def associator(self,listAuthorizedCatRef, *listMontage): #[Piece 1 Piece2, Te, Piece3,Piece4][te,Piee5,Piece6]...
        ...
        for listPieces in listMontage[0]:
            ...
            ## Si on a une piece dans la liste de bifurcation il faut poser la question de qui est la piece precedente
            if len(noeudPiece)>=1:
            ...
            for i in range(offset,len(listPieces)-1):
                ...
                listConn=[(ipieceActive, ipieceNext) for ipieceActive in range(1,pieceActive.nbConnex+1) for ipieceNext in range(1,pieceNext.nbConnex+1) if pieceActive.isCatConnexAuthorized(ipieceActive,pieceNext,ipieceNext,self.listAuthorizedCatRef) and not pieceActive.__dict__[pieceConnexCat[ipieceActive-1]] and not pieceNext.__dict__[pieceNextConnexCat[ipieceNext-1]]]
                ...
                ##TEST
                print(f'PIECE {pieceActive.name}#{pieceActive.ref} CONN{listConn[0]} -to- PIECE {pieceNext.name}#{pieceNext.ref} CONN{listConn[1]}')
                ##
    
    def searchPiece(self,nom,dataBasePandas):
...
Constructor(["BU","BE","TÉ EMBOITEMENT"],["BE","BU","TÉ EMBOITEMENT"])