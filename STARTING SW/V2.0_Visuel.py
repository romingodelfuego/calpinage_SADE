...
class LeftFrame(tk.Frame):  # Inherit from tk.Frame
    ...
    def refreshButton(self):
        ...
        for pieceName in self.piece_names:
            if BRAIN.searchPiece(pieceName, db_Connexions)[1][0] == self.cat_selected.get():
                tk.Button(self.buttonArea, text=pieceName,command=lambda pieceNameLambda=pieceName: self.addToSeriesEltSelec(pieceNameLambda,self.rowSelected)).pack(pady=10,expand=False) 
                print("REFRESHED")
