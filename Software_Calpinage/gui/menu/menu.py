...
from gui.menu.menu_commands import * 

def createMenu(mainWindow,leftFrame,infoPieceFrame,middleFrame,bottomFrame,selectorFrame,buttonFrame,sharedVar):
    ...
    menu_file.add_separator()
    menu_file.add_command(label="Export Project", accelerator="CTRL+SHIFT+E",underline=0, command=lambda: export_table(sharedVar))
    menu_file.add_separator()
    menu_file.add_command(label="Exit", underline=1, command=mainWindow.destroy)
